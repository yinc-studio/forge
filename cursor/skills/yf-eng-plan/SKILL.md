...
**Adversarial review:** launch one `gpt-5.6-sol-medium` `generalPurpose` reviewer with the product-spec **path only**. The reviewer verifies against the codebase and docs via its own `cursor-grok-4.5-high` subagents, edits the file directly; apply the acceptance rule above.
