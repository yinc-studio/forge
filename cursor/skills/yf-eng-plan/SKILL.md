...
## Models (Task allowlist — exact slugs)

| Work | `model` | `subagent_type` |
| ---- | ------- | ----------------- |
| Research (code, specs, ADRs) | `cursor-grok-4.5-high` | `explore` |
| Design / wireframes | `cursor-grok-4.5-high` | `explore` or `generalPurpose` |
| Adversarial review | `claude-fable-5-thinking-high` | `generalPurpose` |
| Reviewer's nested verification | `cursor-grok-4.5-high` | `explore` |
...
5. In **Technical**, keep **Implementation sequence** readable: one plain-English **Intent** line per phase in the body; full per-phase contract fields inline under each phase or in an **Appendix — implementation contract** when the sequence is long — never duplicate the same phase twice.
...
Before drafting product, and again before drafting technical, launch **light** `explore` subagents in parallel (`cursor-grok-4.5-high`). Every run, including thin product work — do not skip because the task "looks obvious."
...
**Design drafts (where UI applies):** rough wireframes in **Design drafts / wireframes**, via `cursor-grok-4.5-high` Task subagents. ASCII or mermaid is fine — hierarchy and key states (empty, loading, error, populated), not pixel detail.

**Adversarial review:** launch one `claude-fable-5-thinking-high` `generalPurpose` reviewer with the wiki spec **path only**. The reviewer verifies against codebase and docs via its own `cursor-grok-4.5-high` `explore` subagents, enforces **Writing for human review**, attacks the **Product** half, must not invent Technical content, and edits the file directly. Apply the acceptance rule above.
...
**Adversarial review:** launch one `claude-fable-5-thinking-high` `generalPurpose` reviewer with the wiki spec **path only**. The reviewer trusts nothing in the document, verifies against code and docs via its own `cursor-grok-4.5-high` `explore` subagents, **edits only Technical** — Product is signed off and frozen — and enforces **Writing for human review**. Attack weak phases (missing Validation, vague Acceptance, ungrounded Docs expected, unreadable writing). Apply the acceptance rule, treating signed-off Product as an explicit user decision.
