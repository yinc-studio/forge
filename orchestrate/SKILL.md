---
name: orchestrate
description: "Coordinate the work by routing each piece of a task to a subagent backed by the right model, then integrating the results. Use when the user runs /orchestrate, asks to orchestrate subagents, or wants work delegated and managed across multiple specialized agents."
...
2. For each unit, launch a subagent via the Agent tool with the model from the routing table below. Pass the `model` parameter explicitly.
3. Run independent units in parallel — batch multiple Agent calls in one message.
4. Review each subagent's output before integrating. You own the final result.
5. Repeat until the task is complete.

## Routing table

Map each unit of work to a model. Always pass the listed slug as the Agent tool `model` parameter.
...
| Reviewing code | `opus` |
| Reviewing plans | `opus` |
| Reviewing writing | `opus` |
| Writing | `sonnet` |
| Planning, plan documents | `opus` |
...
- **Create vs. review**: creating and reviewing writing or plans both go to `opus`. Pair them — have one subagent create, another review, when quality matters.
